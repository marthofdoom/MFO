...
- **Fixed a follower dropping their only weapon.** Every actor carries a hidden "unarmed" weapon. The rule that clears a follower's off role weapons after a fight counted those fists as a weapon in role, so a follower whose one real weapon was off role could drop it and fight bare handed. Fists no longer count. A follower keeps their only weapon even when it is the wrong class for their skills. The exception is a follower with unarmed perks placed through the Progression board. They can fight with their fists and the shed treats their weapons accordingly. The decision is written to the log.
