...
- **Followers' gear is now declared to Harbinger, which keeps the game from putting other things on them.** With Harbinger installed, MFO tells it what each follower wears: the weapons in their hands, their ammo, the armor MFO picked and everything else they have on. Harbinger equips that set and refuses any other equip the game tries on that follower. So a dual wielder no longer gets a shield shoved back into the left hand by their own combat AI a second after MFO put the second weapon there, and a follower stops swapping back into the outfit the game wants. The first build only logs what it would have refused, so you can check the log before it is switched on for real. MFO without Harbinger equips gear directly, exactly as before. There is a toggle (on by default) if you want the old way.
