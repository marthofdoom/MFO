...
- **Loose gems always go into a free socket on worn gear.** A follower with a gem in his pack and an empty socket on something he wears now fills it every time. The pass only looks at gear MEO can socket and follows MEO's own rules, so a support gem waits for a two socket item and a weapon gem never tries a ring. When a gem cannot be placed the log says why in one line.
