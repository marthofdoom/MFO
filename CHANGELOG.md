...
- **Followers dual wield when their perks say so.** A follower whose perks lean toward a weapon in the left hand now fights with two one handed weapons. They draw their best one hander in the right and the next best in the left, judged by the same perk preference. A follower whose perks lean toward a shield draws their best shield instead. No perks either way means nothing changes. The left hand always gives way to a spell, so a follower who casts keeps casting, and the weapon comes back when the cast is done. The left hand only pairs what the follower already carries. A second one hander is not looted for them yet, and a spare one hander that is not their main weapon still sells at the next vendor. This reads the perks themselves, so it works with any perk overhaul.
