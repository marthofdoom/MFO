...
- **The log now counts every attack move a follower makes in a fight.** One line per follower per fight lists each attack, power attack, bash and swing the follower's own animation played, by name, with how long he stood next to his target with weapons out and did nothing. It also names which attack frameworks are loaded. This is for reading the log after a fight where a follower keeps doing the same move. It changes nothing in the game. Off with bAttackObserve = 0 in MFO.ini.
