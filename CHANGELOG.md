...
- **A follower stuck at a gate skips what is behind it and keeps looting.** When a follower walks into a closed gate or a jammed doorway, the game holds him in place (it calls this Movement Blocked). He used to stand there for up to 45 seconds and then try the next item the same way, into the same gate. Now after 3 seconds of that he skips the item, and the other items in the room past the block, and goes on to the next one he can reach. He only comes back to you when nothing he can reach is left.
- **Skipped items come back when the way might be open.** Pull the lever, open or close a door or gate near them, or leave and come back, and the items behind it are back on his list. After a minute they come back anyway, in case the block was just someone standing in the doorway.
- **With Harbinger, MFO no longer pokes a loot trip Harbinger is running.** On a trip that goes through Harbinger, MFO used to push its own package re-check at the follower when the walk had not started. That is Harbinger's job. If the trip has not started within 3 seconds, MFO moves on to the next item through Harbinger instead.
