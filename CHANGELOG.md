...
- **Existing saves are corrected once.** The first time you load an older save, each leveling follower's past growth is recomputed at the player's rate and the extra comes back off. He never drops below where he was when he joined. The log prints one line per follower with the old and new numbers. Saves made after that carry the new record and are never corrected again. (Load the save in an older MFO and it skips the progression record, the same as any newer save.)
