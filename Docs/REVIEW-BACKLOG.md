...
### MFO-B98 (SEV-4, author-raised, pre-existing) -- retreat: a follower who dies mid-retreat keeps his hold
Raised by the author of `fix/mfo-retreat` (ClickUp 86e3erv94), 2026-09-25, NOT by a reviewer. Finding: `Scheduler::Tick`'s dead/disabled early return runs before `ServiceRetreat`, so a follower who dies or is disabled while retreating keeps his `g_retreatHolds` entry; on the APMF road `Pump()` keeps refreshing his ch.9 offer until load/dismissal, and on resurrection the travel timeout fires on his first service. Reasoning: pre-existing (the old global `g_retreatHold` leaked the same way, and there it blocked every OTHER follower's retreat too); per-follower holds shrink it to the dead follower alone. Fix shape: `Packages::RetreatClear("dead", f)` in the dead/disabled branch. Left out of 86e3erv94 because the brief names exactly three end conditions.
