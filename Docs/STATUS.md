...
>
> **▶ IN FLIGHT (2026-09-02, branch `feat/apmf-cast`, NOT merged — field-test first): APMF Phase 3, first MFO→APMF integration (cast-selection slice).** APMF got an append-only ABI **v2** (commit `ba958e2` on APMF `main`, CI-green): new `RequestEx(actor,intent,basis,const APMF_Param*)` on a prefix-extension `APMF_API_v2` (`kABIVersion=2`); `APMF_Param{form,fval,ival}` POD; cast-select now uses `param.form` as the spell (Firebolt fallback), combat-target as the target. MFO side: `native/APMF_API.h` copied byte-identical (frozen external contract, like MEO_API.h — CLAUDE.md #5); new `APMFBridge` (`native/APMFBridge.{h,cpp}`) queries APMF at kDataLoaded via `GetModuleHandleA("APMF.dll")`+`GetProcAddress("APMF_GetInterface")` (hand-declared Win32 — `<windows.h>` banned), **null-degrades** if APMF absent (one log line, MFO casting byte-identical). Wired ONE channel: `Actuation::Fire`'s cast-op block calls `APMFBridge::SelectSpell(follower, spell)` so the follower's OWN AI selects+casts the RIGHT gambit spell (the [[cast-gambit-spell-choice-not-enforced]] gap) — a SELECTION layer ON TOP of MFO's forced `CastSpellImmediate` delivery, never replacing it. Lifecycle = refresh-per-tick + per-pump expiry sweep (`APMFBridge::Tick` in Diagnostics, 500ms) + `ClearTransientState` at kPreLoadGame. Gated `bApmfCast` (default ON, inert without APMF). **NO save/co-save change; MFO frozen contracts untouched.** CI: MFO branch green (see run below). **FIELD-TEST on Tuxborn BEFORE merge** (deck needs APMF.dll installed): a follower with a "cast X" gambit should show `[apmf] interface acquired` + `[ch.8] … selection := <X>` and cast X; without APMF.dll, `[apmf] interface absent` + unchanged casting. Do NOT merge to main / do NOT cut until field-proven.
>
