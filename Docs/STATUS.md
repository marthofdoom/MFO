...
>
> **Last updated:** 2026-08-25 (v1.0.67 SHIPPED — follower economy BUY + mage dress-up + spell-tome buy/learn + gem ungem-then-sell). **Latest public:** v1.0.67 — https://github.com/marthofdoom/MFO/releases/tag/v1.0.67 , DLL `b5a210bc`, commit `4e7acc71`, tag pushed + GitHub Release live (Nexus upload = marth's manual step). Prior public: v1.0.66 (economy default on). The PROGRESSION ADD-ON versions independently (latest v1.0.1, DLL unchanged) and ships as its own zip (both `.esl` and a Vortex `.esp` variant). **NEXT BUILD = gem-autonomy** (default socket-CONSERVATION reconcile-pass + "MEO Aware Followers" effect-aware toggle, mage-clothing-oscillation fix BUNDLED in). **THEN HMS class-redistribution** (v1.0.68 — design pass in progress). See the ✅ v1.0.67 block below the field-session queue for the shipped/queued detail. **⏳ v1.0.68 CANDIDATE BUILT + CI-GREEN (2026-08-25), pending field-test then cut.** Branch `integration/v1.0.68` (`f0f8326`), DLL `84273fa7`, CI `32876928104` green, audit_mcm PASS (51 controls). Merges TWO reviewed+CI-green branches: gem-autonomy + mage-clothing (`worktree-agent-a0eadc07b13f00c07`, 3 Fable findings fixed) and HMS class-redistribution (`worktree-agent-a93f0cca5efba4208`, co-save reviewed SAFE + logic fixes incl. the critical signed-delta budget + the converging-target/semi-permanent-skew refinements + the decay-reachability fix). DEPLOY: custom-modlist staged (`84273fa7`, waits on syncthing → deck awake); Tuxborn scp SKIPPED (deck asleep) — re-run when awake. Gem tiers need MEO ABI v3 to exercise in-game; HMS writes an `[hms]` probe per level-up award. TO CUT: field-confirm → merge `integration/v1.0.68` to main → bump VERSION 1.0.67→1.0.68 → CHANGELOG (marth voice) → release.sh two-phase.
