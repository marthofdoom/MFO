...
- `Logistics.cpp` (2450 after loot M1; 50 lines of headroom under the cap) — core tick: `ServiceFollower` (`:869`, INCLUDING the
...
- `Logistics_internal.h` (1276 after loot M1, which put the GATED records + `GatedNow`/`MarkGated` here) — shared substrate: all `g_*` maps/state
...
- **LOOT ROUND M1 (2026-09-24, `fix/mfo-loot-m1`; design
  ...
  - **MOVEMENT BLOCKED BACKSTOP** (`Logistics.cpp:~1380`, runs AFTER arrival and BEFORE the
    ...
    `>= 3 s`: `MarkGated` (target + the items behind the same block), Holding, next item
    this tick; release ("batch done") ONLY when the scan finds nothing valid.
  - **GATED** (`Logistics_internal.h` `kBlockedGate:~806`, `GatedNow:~957`, `MarkGated:~975`,
    ...
    `TESCellAttachDetachEvent` (per-reference event), `kGateHold` 60 s age-out
    (`PruneGates`, worker tick; the recovery for a false gate such as a doorway jam), a
    full 16-entry table, and `ClearGates` on revert. NOT in `g_travelFailed`: the idle
  ...
  - **Harbinger rule on CH19 legs**: the theft guard never posts MFO's own `EvaluatePackage`
    ...
    once (transient skip) and the Holding scan re-points it through `ClaimLootTravel` or
    releases it. The ch.9 road (`apmfLeg`) is unchanged.
  - **What breaks:** moving the MB block after the theft guard re-opens the 10 s grace +
    ...
    "emptied" ref and the follower ping-pongs.
