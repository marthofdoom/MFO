...
module. Module layout:
  ...
  - `logistics/Lockpick.cpp` (748, NEW LP-M1 2026-09-25) = follower LOCKPICKING of chests: see the
...
- **LOCKPICK, chests (LP-M1, 2026-09-25, `feat/mfo-lockpick`; ClickUp 86e3edgha; design
  ...
  the-lock. Pieces:
  - **Gate** `Lockpick::Admit` (`Lockpick.cpp:445`, called by the scan `LootScan.cpp:377` after the
    ...
    simFail / tooLong / idleEnded / unlockFailed, lifted when his lockpick count or skill rises),
    and the session-wide INERT cases (`InertReason` `:422`: no main-thread pump / Harbinger absent,
  ...
  - **Arrival** (`Service.cpp:760`, before StripCorpse): `Lockpick::Step` (`Lockpick.cpp:488`) is a
    ...
    SIMULATE (`Simulate` `:252`, seeded per follower+lock; engine formulas + the one modelled
    constant `kBindReactSec`) -> a window that does not fit the excursion cap is not started
    (noTime; tooLong when it exceeds the whole cap) -> `APMFBridge::ClaimLockpickHold` (ch.1 +
    ch.12 v2 IdleLockPick at the lock) -> the unpaused pick clock runs only once Harbinger reads
    the idle claim LIVE; one `ReplayLockpickIdle` per simulated pick break -> at the window's end
    ONE `MainThread::Post`: RemoveItem of the broken picks from the FOLLOWER, then (success) the
    engine Unlock `UnlockOnMain` (`:155`, `REL::RelocationID(19821, 20226)`, row `Lockpick.Unlock`,
    `SeatVerified`) -> the next step sees the ref unlocked -> kProceed -> the transfer.
  - **Lifecycle:** `SweepStale` (`:712`, top of `ServiceFollower` `Service.cpp:192`) abandons a job
    whose slot no longer targets its lock or whose arrival step stopped for `kStaleFloor` (3 s);
  ...
  - **What breaks:** removing the `LootHere` lock refusal re-opens loot-through-the-lock on every
    ...
    the idle claim is LIVE (the clock would run while nothing plays); holding a job after its
...
- **Module layout (wave-1 subsystem-folder split, 2026-09-24, a pure move proven function by
  ...
  - **LOCKPICK HOLD (LP-M1, ABI v17, 2026-09-25)** in `apmf/Excursion.cpp` (now 745): file-local
    `g_pickHolds` (`:608`, guarded by `g_mx`), one `LockpickHold` per follower = a ch.12 v2 idle
    ...
    (`g_idleV2Refused` `:613`, session-stable). `ClaimLockpickHold` (`:637`), `ReplayLockpickIdle`
    (`:680`, a v3 Repoint with the same idle = one more PlayIdle; the caller's pick-break events
    only, never a timer), `LockpickHoldStateOf` (`:695`, Pending / Live / Ended with the entry
    table's never-live floors: 15 unpaused polls AND 2 s; an end is Released at once, idle first
    then the stand-still), `ReleaseLockpickHold` (`:729`), `ReleaseAllLockpickHolds` (`:738`).
